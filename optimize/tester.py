...
output = os.popen("head -1 final")
lines = output.read()
bestparams = lines.split()[2:]
print(bestparams)
...
for i in range(len(job.params_Names)):
        FFdata = FFdata.replace(str(job.params_Names[i]) + " ", '{0:.4f}'.format(float(bestparams[i])))

with open(os.path.join(PWD, 'ffield.reax.now'), 'w') as tempfile:
